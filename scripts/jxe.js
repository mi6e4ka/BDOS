...
function createButton(appName, id, text='Кнопка', f='push(id)', x=0, y=0){
	insertWindow(appName,'<button style="position: absolute; top: '+y+'; left: '+x+'" onclick=\''+f+'\'>'+text+'</button>');
}
function createLable(appName, id, text='Тут надпись', x=0, y=0){
	insertWindow(appName,'<span style="position: absolute; top: '+y+'; left: '+x+'">'+text+'</span>');
}
// -- //
